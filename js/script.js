...
$.ajax({
  ...
  success: function(data) {
    $(data).find('dkpentry').each(function() {
      let dkp_player = $(this).find('player').text()
      let dkp_class = $(this).find('class').text().toLowerCase()
      let dkp_current = $(this).find('dkp').text()
...
$.ajax({
  ...
  success: function(data) {
    $(data).find('lootentry').each(function() {
      let loot_player = $(this).find('player').text()
      let loot_itemname = $(this).find('itemname').text()
      let loot_itemnum = $(this).find('itemnumber').text()
      let loot_zone = $(this).find('zone').text()
      let loot_boss = $(this).find('boss').text()
      let loot_cost = $(this).find('cost').text()
...
$.ajax({
  ...
  success: function(data) {
    $(data).find('historyentry').each(function() {
      let hist_players = $(this).find('playerstring').text()
      let hist_dkp = $(this).find('dkp').text()
      let hist_time = $(this).find('timestamp').text()
      let hist_reason = $(this).find('reason').text()
